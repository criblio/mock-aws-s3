...
function applyBasePath (search) {
	...
	var ret = _.mapObject(search, function (value, key) {
		if (_.indexOf(modifyKeys, key) === -1) {
			...
			return config.basePath + path.sep + value
		}
	})
...
function FakeStream (search) {
	this.src = search.Bucket + path.sep + search.Key
}
...
S3Mock.prototype = {
	...
	listObjects: function (search, callback) {
		...
		var filteredFiles = _.filter(files, function (file) {
			return !search.Prefix || file.replace(search.Bucket + path.sep, '').indexOf(search.Prefix.replace(/\//g, path.sep)) === 0
		})
		var start = 0
		var truncated = false
		...
		if (search.Marker) {
			...
			var markerFile = _(filteredFiles).find(function (file) {
				var marker = search.Bucket + path.sep + search.Marker.replace(/\//g, path.sep)
		...
		var result = {
			Contents: _.map(filteredFiles, function (_path) {
				var stat = fs.statSync(_path)

				return {
					Key: _path.replace(search.Bucket + path.sep, '').split(path.sep).join('/'),
					ETag: '"' + crypto.createHash('md5').update(fs.readFileSync(_path)).digest('hex') + '"',
					LastModified: stat.mtime,
					Size: stat.size
				}
			}),
			CommonPrefixes: _.reduce(filteredFiles, function (prefixes, _path) {
				var prefix = _path
					.replace(search.Bucket + path.sep, '')
					.split(path.sep)
					.slice(0, -1)
					.join('/')
					.concat('/')
	...
	deleteObjects: function (search, callback) {
		...
		_.each(search.Delete.Objects, function (file) {
			if (fs.existsSync(search.Bucket + path.sep + file.Key)) {
				deleted.push(file)
				fs.unlinkSync(search.Bucket + path.sep + file.Key)
			}
			else {
				errors.push(file)
			}
	...
	deleteObject: function (search, callback) {
		...
		if (fs.existsSync(search.Bucket + path.sep + search.Key)) {
			fs.unlinkSync(search.Bucket + path.sep + search.Key)
			callback(null, true)
		}
		else {
			callback(null, {})
	...
	headObject: function (search, callback) {
		...
		if (!callback) {
			...
			fs.readFile(search.Bucket + path.sep + search.Key, function (err, data) {
				if (!err) {
					var props = {
						Key: search.Key,
	...
	getObject: function (search, callback) {
		...
		if (!callback) {
			...
			var _path = search.Bucket + path.sep + search.Key
	...
	copyObject: function (search, callback) {
		...
		fs.mkdirsSync(path.dirname(search.Bucket + path.sep + search.Key))

		fs.copy(decodeURIComponent(search.CopySource), search.Bucket + path.sep + search.Key, function (err, data) {
			callback(err, search)
		})
	},
	...
	putObject: function (search, callback) {
		...
		var dest = search.Bucket + path.sep + search.Key
